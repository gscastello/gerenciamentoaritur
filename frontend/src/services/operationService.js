...
export const operationService = {
  ...
  async removeFuelRecord(id) {
    ...
    if (error)
      throw new ServiceError(`removeFuelRecord: ${error.message}`, {
        cause: error,
        retryable: isNetworkish(error),
      });
    if (!data?.success)
      throw new ServiceError(data?.message || "Não foi possível remover o abastecimento.", {
        retryable: false,
      });
    return data;
  },
  ...
  /** `vehicleId` omitido busca de toda a frota (comparativo). */
  async listMaintenance({ vehicleId, from, to } = {}) {
    let q = supabase
      .from("maintenance")
      .select("*")
      .is("deleted_at", null)
    ...
    return handle(q, "listMaintenance");
  },
  ...
  async updateMaintenance(id, fields) {
    const actor = await getCurrentUserId();
    return handle(
      supabase
        .from("maintenance")
  ...
  async removeMaintenance(id) {
    ...
    if (error)
      throw new ServiceError(`removeMaintenance: ${error.message}`, {
        cause: error,
        retryable: isNetworkish(error),
      });
    if (!data?.success)
      throw new ServiceError(data?.message || "Não foi possível remover a manutenção.", {
        retryable: false,
      });
    return data;
  },
  ...
  async getMaintenanceStatus(maintenanceRecord) {
    const { data, error } = await supabase
      ...
      .is("deleted_at", null);
    if (error)
      throw new ServiceError(`getMaintenanceStatus: ${error.message}`, {
        cause: error,
        retryable: isNetworkish(error),
      });
    const kmSince = (data || []).reduce((s, r) => s + Number(r.km || 0), 0);
    const pct = maintenanceRecord.interval_km
      ? Math.min(100, Math.round((kmSince / maintenanceRecord.interval_km) * 100))
      : 0;
