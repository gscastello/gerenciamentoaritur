...
export const vehiclesService = {
  ...
  async setDefault(vehicleId) {
    ...
    await handle(
      supabase
        .from("vehicles")
        .update({ is_default: false, updated_by: actor })
        .eq("is_default", true),
      "setDefault(unset)",
    );
    return handle(
      supabase
        .from("vehicles")
        .update({ is_default: true, updated_by: actor })
        .eq("id", vehicleId)
        .select()
        .single(),
      "setDefault(set)",
  ...
  async addVehicle({ name, plate, type, capacity }) {
    const actor = await getCurrentUserId();
    return handle(
      supabase
        .from("vehicles")
        .insert({ name, plate, type, capacity, created_by: actor })
        .select()
        .single(),
      "addVehicle",
    );
  ...
  /** Soft-delete via RPC (bloqueia se for o veículo padrão — ver
   *  database/46-veiculos-e-tipos-manutencao.sql). */
  async removeVehicle(vehicleId) {
    const { data, error } = await supabase.rpc("rpc_soft_delete_vehicle", { p_id: vehicleId });
    if (error)
      throw new ServiceError(`removeVehicle: ${error.message}`, {
        cause: error,
        retryable: isNetworkish(error),
      });
    if (!data?.success)
      throw new ServiceError(data?.message || "Não foi possível remover o veículo.", {
        retryable: false,
      });
    return data;
  },
  ...
  async addDriver({ name, phone }) {
    const actor = await getCurrentUserId();
    return handle(
      supabase.from("drivers").insert({ name, phone, created_by: actor }).select().single(),
      "addDriver",
    );
  },
  ...
  async removeDriver(driverId) {
    ...
    if (error)
      throw new ServiceError(`removeDriver: ${error.message}`, {
        cause: error,
        retryable: isNetworkish(error),
      });
    if (!data?.success)
      throw new ServiceError(data?.message || "Não foi possível remover o motorista.", {
        retryable: false,
      });
    return data;
  },
